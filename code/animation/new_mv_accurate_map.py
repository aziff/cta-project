...
def get_x_y(info_array):
    #make a copy and delete the weird extraneous rows
    copy = []
    ...
    #Loop through x and y and convert to plot-able coordinates
    i = 1
    order = 1
    for row in range(1,len(copy)):
        ...
        if info_array[row][4] == 'S':
            ...
            #HERE IS THE PART WE NEED TO EDIT NOW THAT WE HAVE THE DATA 
...
patch1 = plt.Circle((.5, -.5), 0.02, fc='y')
patch2 = plt.Circle((.5, -.5), 0.04, fc='b')
patch3 = plt.Circle((.5, -.5), 0.05, fc='g')
patch4 = plt.Circle((.5, -.5), 0.07, fc='c')
patch5 = plt.Circle((.5, -.5), 0.09, fc='b')
patch6 = plt.Circle((.5, -.5), 0.1, fc='b')
